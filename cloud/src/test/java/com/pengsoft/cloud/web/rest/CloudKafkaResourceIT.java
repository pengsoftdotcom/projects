...
import com.pengsoft.cloud.config.KafkaProperties;
...
class CloudKafkaResourceIT {
    ...
    void setup() {
        ...
        CloudKafkaResource kafkaResource = new CloudKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cloud-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cloud-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
