...
(function(window) {
    ...
    calendar.prototype._init = function(options) {
            ...
            this.tableContainer = this.options.container;
            ...
            for (var i = 0; i < tdList.length; i++) {
                if (tdList[i].getAttribute('date')) {
                    tdList[i].addEventListener('tap', function(e) {
                        ...
                        self.options.dateActive(this.getAttribute('date'));
                    })
                }
            }
        }
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    if (s >= firstDay && d <= monthLen) {
                        if (_d > d & this.options.beforeStartDateClick == false) {
                            html += '<td date="' + year + '-' + this._foo(month) + '-' + this._foo(d) + '" class="before-day" click-status="' + this.options.beforeStartDateClick + '">';
                        } else {
                            if (col == 0 || col == 6) {
