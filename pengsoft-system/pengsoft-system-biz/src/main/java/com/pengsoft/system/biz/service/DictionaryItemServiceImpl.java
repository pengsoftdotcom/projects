...
@Primary
@Service
public class DictionaryItemServiceImpl extends TreeBeanServiceImpl<DictionaryItemRepository, DictionaryItem, String> implements DictionaryItemService {
    ...
    public DictionaryItem save(final DictionaryItem item) {
        findOneByTypeAndParentAndCode(item.getType(), item.getParent(), item.getCode()).ifPresent(source -> {
            if (EntityUtils.ne(source, item)) {
                throw exceptions.constraintViolated("code", item.getCode());
            }
        });
